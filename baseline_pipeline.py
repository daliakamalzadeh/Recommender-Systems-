...
# Leave-one-last-interaction-out validation split per user
def chronological_split(train: pd.DataFrame, max_val_users: int | None = DEFAULT_MAX_VAL_USERS) -> Tuple[pd.DataFrame, pd.DataFrame]:

    df = train.sort_values(["user_id", "timestamp"])
    val = df.groupby("user_id", as_index=False).tail(1)
    train_split = df.drop(val.index)

    valid_users = set(train_split["user_id"].unique())
    val = val[val["user_id"].isin(valid_users)]

    if max_val_users is not None and len(val) > max_val_users:
        val = val.sample(max_val_users, random_state=RANDOM_STATE)
...
# BM25-style popularity baseline
def bm25_weight_matrix(matrix: csr_matrix, k1: float = 1.2, b: float = 0.75) -> csr_matrix: # Apply BM25 weighting to the user-item interaction matrix
    ...
    # Item document frequency: number of users who interacted with item
...
# SVD and NMF matrix-factorization baselines
...
# BPR matrix factorization with negative sampling
def sample_negative_item(rng: np.random.Generator, n_items: int, seen: Set[int]) -> int: #Uniform negative sampling from items not interacted with by the user
    while True:
...
def train_bpr(
    ...
    for epoch in range(epochs):
        ...
        for u, i in interactions:
            ...
            # sigmoid(-x) = 1 / (1 + exp(x)); clip avoids overflow.
