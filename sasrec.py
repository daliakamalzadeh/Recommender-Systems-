"""
SASRec — sequential recommender (Task 2 / "the model") for Group 30.

Self-Attentive Sequential Recommendation (Kang & McAuley, ICDM 2018).
Trained ONLY on train.csv. Produces top-10 recommendations per user for the
Kaggle submission, scored by Recall@10.

Pipeline (raw data -> submission):
  1. Load + clean train.csv (numeric coercion, drop NA, dedup, chronological sort).
  2. Map item_id -> contiguous index [1..N]; index 0 is the padding token.
  3. Build one chronological item sequence per user.
  4. Leave-one-out split: each user's last item is the validation target; the
     rest is the training history. Model selection uses local Recall@10 only —
     the public test set is never read.
  5. Train SASRec (causal self-attention) with next-item prediction. Loss is
     full-vocabulary cross-entropy by default (every other item is an implicit
     negative); sampled-negative BCE is available via --neg-mode sampled.
  6. Early-stop on validation Recall@10.
  7. Inference: feed each submission user's full observed history (most recent
     `max_len` items), read the final-position logits, mask already-seen items,
     and take the top-10 by score.

Reproducible: fixed seeds, deterministic split, pinned behaviour. Runs on GPU
automatically when CUDA is available (use --device to force cpu/cuda).

Usage:
  python sasrec.py --data-dir . --output submission_sasrec.csv
  python sasrec.py --data-dir . --epochs 200 --hidden 64 --blocks 2 --amp
"""
...
TOP_K = 10
PAD = 0  # reserved padding / "no item" index
...
def set_seed(seed: int) -> None:
    ...
    # Deterministic cuDNN; small speed cost, big reproducibility win.
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.benchmark = False


# --------------------------------------------------------------------------- #
# Data loading / preprocessing (mirrors the team's baseline conventions)
# --------------------------------------------------------------------------- #
@dataclass
class Dataset:
    user_sequences: Dict[int, List[int]]   # user_id -> chronological item indices (full history)
    train_seq: Dict[int, List[int]]        # user_id -> history WITHOUT the held-out items
    val_targets: Dict[int, set]            # user_id -> set of held-out item indices (leave-N-out)
    item_id_to_idx: Dict[int, int]         # raw item_id -> contiguous index [1..N]
    idx_to_item_id: np.ndarray             # index -> raw item_id (index 0 unused)
    n_items: int                           # number of real items (excludes PAD)
    popularity: List[int]                  # item indices sorted by descending frequency (fallback)
    pop_scores: np.ndarray                 # z-scored log-popularity per item index (for blending)


def clean_train(train: pd.DataFrame) -> pd.DataFrame:
    """Coerce types, drop invalid rows, keep the most recent duplicate, sort."""
    ...
    # Collapse repeated (user, item) events to their most recent occurrence.
    df = df.sort_values(["user_id", "item_id", "timestamp"])
    df = df.drop_duplicates(subset=["user_id", "item_id"], keep="last")
    return df.sort_values(["user_id", "timestamp"]).reset_index(drop=True)


def build_dataset(train_clean: pd.DataFrame, val_holdout: int = 2) -> Dataset:
    """Build sequences and a leave-N-out validation split.

    `val_holdout` items per user are held out as validation targets to mirror the
    Kaggle metric (which scores ~2 future interactions per user). Recall@10 is
    then computed over that held-out SET, not a single item.
    """
    # Contiguous item indexing; 0 reserved for padding.
    ...
    # Chronological sequence of item indices per user.
    user_sequences: Dict[int, List[int]] = {}
    for uid, grp in train_clean.groupby("user_id", sort=False):
        seq = [item_id_to_idx[int(i)] for i in grp["item_id"].to_numpy()]
        user_sequences[int(uid)] = seq

    # Leave-N-out: last `val_holdout` items -> validation set; remainder -> history.
    # Users too short to spare N items still contribute their full history to training.
    train_seq: Dict[int, List[int]] = {}
    val_targets: Dict[int, set] = {}
    for uid, seq in user_sequences.items():
        if len(seq) > val_holdout:
            train_seq[uid] = seq[:-val_holdout]
            val_targets[uid] = set(seq[-val_holdout:])
        else:
            train_seq[uid] = seq

    # Popularity ranking + z-scored log-popularity vector (for inference blending).
    counts = train_clean["item_id"].value_counts()
    ...
    real = pop_scores[1:]
    pop_scores[1:] = (real - real.mean()) / (real.std() + 1e-6)  # z-score real items
...
# --------------------------------------------------------------------------- #
# Torch dataset: next-item prediction sequences (left-padded)
# --------------------------------------------------------------------------- #
class SeqDataset(torch.utils.data.Dataset):
    """
    For each user with a training history of length >= 2, emit a left-padded
    (input, target) pair where target is the input shifted by one position
    (next-item prediction). PAD positions carry target PAD and are ignored.
    """
    ...
    def __getitem__(self, i: int):
        seq = self.samples[i][-(self.max_len + 1):]  # keep at most max_len+1 to form len-max_len IO
        inp = seq[:-1]
        tgt = seq[1:]
...
class SASRec(nn.Module):
    def __init__(self, n_items: int, max_len: int, hidden: int = 64,
        ...
        # +1 for the PAD index at position 0 (zeroed embedding).
    ...
    def encode(self, seq: torch.Tensor) -> torch.Tensor:
        ...
        pad_mask = (seq == PAD)                                   # (B, L) True where padded
        causal = torch.triu(                                     # (L, L) True above diagonal = blocked
        ...
        for ln_a, attn, ln_f, ffn in zip(
            ...
            x = x + a
            x = x + ffn(ln_f(x))
            x = x.masked_fill(pad_mask.unsqueeze(-1), 0.0)       # keep pads at zero
...
def train_one_epoch(model, loader, optimizer, device, neg_mode, num_neg, n_items,
    ...
    for inp, tgt in loader:
        ...
        with torch.autocast(device_type=device.type, enabled=scaler is not None):
            if neg_mode == "full":
                # Cross-entropy over the full item vocabulary; PAD targets ignored.
                logits = model.logits_all(inp)                       # (B, L, V)
                ...
                # Sampled-negative BCE (original SASRec objective).
                reps = model.encode(inp)                             # (B, L, H)
                pos_emb = model.item_emb(tgt)                        # (B, L, H)
                mask = (tgt != PAD).float()                          # (B, L)
                neg = torch.randint(1, n_items + 1, (inp.size(0), inp.size(1), num_neg),
                                    device=device)
                neg_emb = model.item_emb(neg)                        # (B, L, num_neg, H)
                pos_score = (reps * pos_emb).sum(-1)                 # (B, L)
                neg_score = (reps.unsqueeze(2) * neg_emb).sum(-1)    # (B, L, num_neg)
...
def fit(model, loader, data, optimizer, device, *, epochs, patience, eval_every,
    ...
    """Train with early stopping on local Recall@10. Returns (best_recall, history).

    Restores the best-Recall@10 weights into `model` before returning. `history`
    is a list of (epoch, loss, recall, ndcg) tuples for plotting / the report.
    """
...
# --------------------------------------------------------------------------- #
# Evaluation: local Recall@10 / NDCG@10 over the held-out SET (matches Kaggle)
# --------------------------------------------------------------------------- #
def blend_scores(logits: torch.Tensor, pop_t: torch.Tensor, alpha: float) -> torch.Tensor:
    """Per-user z-scored SASRec logits blended with z-scored log-popularity.

    alpha=0 -> pure SASRec; alpha=1 -> pure popularity. Standardising per row puts
    the two signals on a comparable scale before mixing.
    """
    if alpha <= 0:
        return logits
    real = logits[:, 1:]                                  # exclude PAD column
...
def evaluate(model, data: Dataset, device, max_len: int,
    ...
    """Mean Recall@10 / NDCG@10 over each user's held-out set.

    Recall@10 = |top-10 ∩ held-out| / |held-out|  (the Kaggle definition).
    """
    model.eval()
    ...
    for start in range(0, len(users), batch_size):
        ...
        seqs, seens, targets = [], [], []
        for u in batch:
            hist = data.train_seq[u][-max_len:]            # input excludes the held-out items
        ...
        for row, seen, tgt in zip(scores, seens, targets):
            row[list(seen)] = float("-inf")               # never re-recommend seen items
...
def recommend(model, data: Dataset, user_ids: Sequence[int], device, max_len: int,
    ...
    for start in range(0, len(known), batch_size):
        ...
        seqs, seens = [], []
        for u in batch:
            hist = data.user_sequences[u][-max_len:]            # full history (incl. former val items)
            seqs.append([PAD] * (max_len - len(hist)) + hist)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--val-holdout", type=int, default=2,
                   help="items held out per user for validation (match Kaggle ~2)")
...
def main() -> None:
    ...
    data = build_dataset(train_clean, val_holdout=args.val_holdout)
    print(f"users: {len(data.user_sequences)} | items: {data.n_items} | "
          f"val users (leave-{args.val_holdout}-out): {len(data.val_targets)}")

    # Training sequences (exclude each user's held-out items).
    ...
    # --- Train, selecting the epoch on pure-SASRec Recall@10 ----------------
    ...
    # --- Optional: pick the popularity-blend weight on validation -----------
