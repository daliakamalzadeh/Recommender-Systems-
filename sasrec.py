...
def build_dataset(train_clean: pd.DataFrame, val_quantile: float = 0.90,
                  final: bool = False, final_val_quantile: float = 0.98) -> Dataset:
    ...
    cutoff = None
    if final:
        cutoff = float(np.quantile(train_clean["timestamp"].to_numpy(), final_val_quantile))
        for uid, seq_ts in user_seq_ts.items():
            hist = [i for i, ts in seq_ts if ts < cutoff]
            future = {i for i, ts in seq_ts if ts >= cutoff}
            train_seq[uid] = hist if hist else [i for i, _ in seq_ts]
            if hist and future:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--final-val-quantile", type=float, default=0.98,
                   help="shallow holdout quantile used only for early stopping in --final")
...
def main() -> None:
    ...
    data = build_dataset(train_clean, val_quantile=args.val_quantile, final=args.final,
                         final_val_quantile=args.final_val_quantile)
    if args.final:
        print(f"users: {len(data.user_sequences)} | items: {data.n_items} | "
              f"FINAL run: early-stop holdout q={args.final_val_quantile} "
              f"({len(data.val_targets)} val users); full history used at inference")
    else:
        print(f"users: {len(data.user_sequences)} | items: {data.n_items} | "
    ...
    if args.final:
        print(f"Final run: restored best checkpoint, Recall@10 {best_recall:.5f} "
              f"on the q={args.final_val_quantile} holdout.")
    else:
